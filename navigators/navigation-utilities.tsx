...
export function useNavigationPersistence(storage: any, persistenceKey: string) {
    ...
    const onNavigationStateChange = (state) => {
        ...
        if (previousRouteName !== currentRouteName) {
            ...
            runUnprotected(() => {
                getAppStore().currentRoute = currentRouteName
            })

            switch (currentRouteName) {
                case 'wallet-main':
                    events.send(MARKETING_EVENTS.WALLET_ICON)
                    break
                case 'settings-main':
                    events.send(MARKETING_EVENTS.SETTINGS_ICON)
                    break
                case 'browser':
                    events.send(MARKETING_EVENTS.BROWSER_ICON)
            }
        }
